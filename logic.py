...
class Pokemon:
    ...
    async def feed(self, feed_interval=60, hp_increase=10):
        ...
        if (current_time - self.last_feed_time) > delta_time:
            ...
            return f"Pokémon's health has been restored. Current Health: {self.hp}"
        else:
            return f"You can feed your Pokémon at: {current_time+delta_time}"
    ...
    async def info(self):
        ...
        return f"""Your Pokémon's name: {self.name}
                Pokémon Power: {self.power}
                Pokémon Health: {self.hp}"""
    ...
    async def attack(self, enemy):
        if isinstance(enemy, Wizard):
            chance = randint(1, 5)
            if chance == 1:
                return "The Wizard Pokémon used a shield in battle!"
